...
#define DEFAULT_DISPLAY_MODE	DISPLAY_MODE_IMAGE //DISPLAY_MODE_OVERLAY, DISPLAY_MODE_DEPTH, DISPLAY_MODE_IMAGE
...
void SimpleViewer::Display()
{
	...
	// Subclass draw hook
	XnPoint3D currentPoint = DisplayPostDraw();

	// Displays game status (enabled or not)
	gameController.displayGameStatus(getNumOfHands());
	
	//Draw random points to the screen
	gameController.displayRandomPoint(currentPoint);
	
	//Draw random bad points to the screen
	gameController.displayRandomBadPoint(currentPoint);
	
	//Display the current result of the game on the screen
